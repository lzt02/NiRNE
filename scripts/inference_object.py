...
def main():
    ...
    x_start_pipeline = YOSONormalsPipeline.from_pretrained(
        'weights/yoso-normal', trust_remote_code=True, variant="fp16", torch_dtype=torch.float16, t_start=0).to(device)
    pipe = NiRNEPipeline.from_pretrained('weights/NiRNE', trust_remote_code=True,
